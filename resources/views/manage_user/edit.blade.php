...
<script type="text/javascript">
  $(document).ready(function(){
   ...
   $('#role').change( function(){
    thisValue = $("#role option:selected").val();
    if(thisValue == "" || thisValue == undefined)
    {
      alert("Please Select Role First ");retur(false);
    }
    getLocations();
  });
  function getLocations() {
    var cat = $('#role option:selected').val();
    $.ajax({
        method: 'POST',
        url: '/users/getLocations',
        dataType: 'html',
        data: { role_id: cat },
        success: function(result) {
            if (result) {
                $('#role_location').html(result);
            }
        },
    });
}
