...
class Util
{
    ...
    */
    ...
    {
        $payment_types = ['cash' => __('lang_v1.cash'), 'card' => __('lang_v1.card'), 'cheque' => __('lang_v1.cheque'), 'bank_transfer' => __('lang_v1.bank_transfer'), 'gift_card' => __('lang_v1.gift_card'), 'coupon' => __('lang_v1.coupon'), 'bonus_points' => 'Bonus Points', 'other' => __('lang_v1.other')];
        // $payment_types = ['cash' => __('lang_v1.cash'), 'gift_card' => __('lang_v1.gift_card'), 'coupon' => __('lang_v1.coupon'), 'force_price' => __('lang_v1.force_price'), 'unknown' => __('lang_v1.unknown')];
    ...
    */
    ...
    public function uploadFileArr($request, $file_name, $dir_name, $index)
    {
        //If app environment is demo return null
        if (config('app.env') == 'demo') {
            return null;
        ...
        if ($request->hasFile($file_name) && $request->file($file_name)[$index]->isValid()) {
            if ($request->file($file_name)[$index]->getSize() <= config('constants.document_size_limit')) {
                $new_file_name = time() . '_' . $request->file($file_name)[$index]->getClientOriginalName();
                if ($request->file($file_name)[$index]->storeAs($dir_name, $new_file_name)) {
                    $uploaded_file_name = $new_file_name;
                }
            }
        }
